...
function [a,tempXprod, ph, theta] = runTheta(a,tempXprod,w,p,stage)
  ...
  for ACh_i = 1:size(AChLvls,2)
    % initialize CA1 activity
    syn.EC3(:,1,ACh_i) = (1 - AChLvls(ACh_i)*Cl) * ((theta.EC(1) .* w.EC) * a.EC(:,stage));
    syn.CA3(:,1,ACh_i) = (1 - AChLvls(ACh_i)*Cr) * ((theta.CA3(1) .* w.CA3) * a.CA3(:,stage));
    
    a.CA1(:,1,ACh_i) = syn.EC3(:,1,ACh_i) + syn.CA3(:,1,ACh_i); % eq 2.4 p.799
    % a.CA1(:,1,ACh_i) = ((theta.EC(1) .* w.EC) * a.EC(:,stage)) + ((theta.CA3(1) .* w.CA3) * a.CA3(:,1)); % eq 2.4 p.799
    
    tempXprod(:,:,1) = (theta.LTP(1) .* a.CA1(:,1,ACh_i)) * a.CA3(:,stage)';
    % run one theta cycle
    for t = 2:p.stepsPerCycle + 1
      ...
      syn.EC3(:,t,ACh_i) = (1 - AChLvls(ACh_i)*Cl) * ((theta.EC(t) .* w.EC) * a.EC(:,stage));
      syn.CA3(:,t,ACh_i) = (1 - AChLvls(ACh_i)*Cr) * ((theta.CA3(t) .* w.CA3) * a.CA3(:,stage));
      
      a.CA1(:,t,ACh_i) = syn.EC3(:,t,ACh_i) + syn.CA3(:,t,ACh_i); % eq 2.4 p.799

      tempXprod(:,:,t,ACh_i) = (theta.LTP(t) .* a.CA1(:,t,ACh_i)) * a.CA3(:,stage)';
